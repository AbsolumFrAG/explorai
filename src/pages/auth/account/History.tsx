...
const History: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchItineraries = async () => {
      try {
        ...
        setError("Échec du chargement des itinéraires");
        setLoading(false);
      }
    };
    ...
    for (let i = 1; i <= totalPages; i++) {
      pages.push(
        <button
          ...
          className={`rounded-lg border px-3 py-1 ${currentPage === i ? "bg-blue-500 text-white" : ""}`}
        >
          {i}
        </button>
      );
  ...
  const handleDelete = async (itineraryId: string) => {
    ...
    try {
      ...
      setError("Échec de la suppression de l'itinéraire");
    }
  };
  ...
  return (
    <div>
      <h2 className="mb-8 text-3xl font-bold text-gray-800 sm:text-5xl">
      ...
      {loading ? (
        ...
        <li className="flex items-center justify-between rounded-2xl border border-gray-300 border-opacity-20 bg-white px-8 py-4 shadow-xl">
          <div className="space-y-2">
            <p className=" rounded-lg px-4 py-2 text-xl font-medium text-gray-500">
        ...
        <ul className="space-y-4">
          {currentItineraries.map((itinerary, id) => (
            <li
              className="flex items-center justify-between space-x-4 rounded-2xl border border-gray-300 border-opacity-20 bg-white px-8 py-4 shadow-xl sm:space-x-2"
              key={id}
            >
