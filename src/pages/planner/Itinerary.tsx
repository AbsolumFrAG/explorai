...
const itineraryReducer = (
  ...
  switch (action.type) {
    ...
    case "TOGGLE_ARRAY_FIELD_ITEM": {
      if (action.field === "activity") {
        const currentActivities = state.activity;
        const valueIndex = currentActivities.indexOf(action.value);
        const updatedArray =
          valueIndex >= 0
            ? currentActivities.filter((_, index) => index !== valueIndex)
            : [...currentActivities, action.value];
        return { ...state, activity: updatedArray };
      }
      return state;
    }
    default:
  ...
  return (
    <>
      ...
      >
          ...
          <div className="grid grid-cols-2 grid-rows-2 gap-4 sm:grid-cols-3">
            {groupOptions.map((group) => (
              <button
                ...
                className={`button h-28 bg-white text-gray-800 duration-75 ${state.group === group.value ? "border-2 border-gray-800" : "border border-gray-300 hover:border-gray-500"}`}
              >
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            {budgetOptions.map((budget) => (
              <button
                ...
                className={`button h-32 bg-white duration-75 ${state.budget === budget.value ? "border-2 border-gray-800" : "border border-gray-300 hover:border-gray-500"}`}
              >
                <div className="space-y-2 text-left">
          ...
          <div className="grid grid-cols-2 grid-rows-3 gap-4 sm:grid-cols-3">
            {activities.map((activity) => (
              <button
                ...
                className={`button h-28 bg-white text-gray-800 duration-75 ${state.activity.includes(activity.value) ? "border-2 border-gray-800" : "border border-gray-300 hover:border-gray-500"}`}
              >
                <div className="space-y-2 text-left">
        ...
        <div
          className={`${hasErrors ? "-translate-y-28" : ""} error fixed bottom-0 left-0 w-full overflow-hidden bg-red-600 bg-opacity-80 px-16 py-4 text-white backdrop-blur-2xl transition-transform duration-300`}
        >
