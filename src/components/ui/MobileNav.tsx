...
const MobileNav = React.memo(() => {
  ...
  return (
    <div
      className={`bg-gray-800 py-6 sm:bg-gradient-to-b sm:from-[#00000060] sm:to-transparent ${
        hasGrayBg ? "bg-gray-800" : ""
      }`}
    >
      <div className="mx-auto flex items-center justify-between px-4">
        <NavLink to="/">
          <img src="/logo.svg" className="w-10" alt="Logo ExplorAI" />
        </NavLink>
      ...
      {isOpen && (
        <div className="fixed left-0 top-0 flex h-screen w-full flex-col items-center justify-center space-y-4 bg-black bg-opacity-50 py-3 text-white backdrop-blur-xl">
          ...
          {renderNavLink("/planner", "Planifier un voyage")}
          <hr className="w-[80%] opacity-30" />
          ...
          {user ? (
            <>
              <hr className="w-[80%] opacity-30" />
              {renderNavLink("/account", "Compte")}
            </>
          ) : (
            <NavLink
              to="/sign"
              onClick={toggleMenu}
              className={`button w-[80%] bg-gradient-to-bl from-sky-500 to-indigo-500 text-center text-2xl font-bold ${
                isHidden ? "hidden" : ""
              }`}
            >
              Se connecter
